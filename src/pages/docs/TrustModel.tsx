...
2. NexArt creates a CER bundle and computes the certificateHash
3. The attestation node signs the certificateHash, producing a receipt
...
2. Compare with the certificateHash in the receipt
...
const TrustModel = () => (
  <>
    ...
    <ol>
      ...
      <li><strong>Node attestation.</strong> An attestation node signs the certificateHash using Ed25519, producing a receipt that binds the hash to a timestamp and node identity.</li>
...
2. NexArt creates a CER bundle and computes the certificateHash (SHA-256)
3. The attestation node signs the certificateHash → produces a receipt
4. The API returns:
   - certificateHash
   - receipt (with timestamp, nodeId, attestorKeyId)
   - signatureB64Url
   - verificationUrl
5. Anyone can verify the record at the verificationUrl or independently`}
    ...
    <ul>
      <li>The CER bundle (including the receipt and signature)</li>
    ...
    <ol>
      ...
      <li>Compare it with the certificateHash in the receipt</li>
      <li>Fetch the node's public key matching the receipt's <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">attestorKeyId</code></li>
