  ...
  <>
    ...
    <div className="mb-6 rounded-lg border border-destructive/40 bg-destructive/5 p-4">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        <code className="bg-muted px-1.5 py-0.5 rounded text-xs font-mono ml-1">verifyProjectBundle()</code> prove
        integrity only. To make an artifact resolvable on
        <code className="bg-muted px-1.5 py-0.5 rounded text-xs font-mono ml-1">verify.nexart.io</code>, you MUST
        register it on the attestation node. See the
        <Link to="/docs/end-to-end-verification" className="text-primary hover:underline ml-1">
    ...
    <p>
      Verification confirms the integrity and authenticity of a Certified Execution Record or Project Bundle. The
      canonical flow is:
    </p>
    <ol>
      <li>An execution happens.</li>
      <li>
        A CER is <strong>sealed</strong> locally (SDK{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">sealCer()</code> or CLI{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">nexart ai seal</code>), binding execution
        metadata to a deterministic{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. This is fully
        offline.
      </li>
      <li>
        The sealed bundle can be verified immediately. Layer 1 (integrity) returns PASS; Layers 2 and 3 return SKIPPED
        because no node attestation is present.
      </li>
      <li>
        Optionally, the CER is <strong>certified</strong> by an attestation node, which adds a signed receipt and a
        verification envelope.
      </li>
      <li>
        A certified bundle can be verified against all three layers (PASS/PASS/PASS) and is resolvable at{" "}
      ...
      <li>
        Optionally, multiple CERs are collected into a{" "}
        <Link to="/docs/concepts/project-bundles" className="text-primary hover:underline">
          Project Bundle
        </Link>{" "}
        with a <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">projectHash</code>.
      </li>
    </ol>
    ...
    <p>
      Verification is split into three independent layers. Each layer protects a distinct property and reports its
      result independently. Envelope failure MUST NOT be reported as integrity failure.
    </p>
    ...
    <p>
      ...
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">context</code> (if present),{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">contextSummary</code> (if present).
    </p>
    <p>
      <strong>Not hashed:</strong>{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">certificateHash</code>,{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">meta</code>,{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">declaration</code>,{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">verificationEnvelope</code>,{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">verificationEnvelopeSignature</code>,{" "}
      <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">receipt</code>, any unknown fields.
    </p>

    <h3 id="layer-2">Layer 2 — Receipt (Node stamp)</h3>
    <p>
      Validates the Ed25519 signed receipt over the{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. Proves the node
      attested the bundle at a specific time. Receipt verification is independent of envelope verification.
    </p>

    <h3 id="layer-3">Layer 3 — Verification Envelope (v0.16.1, v2)</h3>
    <p>
      Ed25519 signature over{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">{`{ attestation, bundle }`}</code> using a
      strict whitelist projection. Proves full bundle integrity under the node's signature beyond the certificateHash.
    </p>
    <ul>
      <li>
        <strong>Attestation projection (5 fields, exact):</strong>{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">attestationId</code>,{" "}
      ...
      <li>
        <strong>Bundle projection:</strong>{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">bundleType</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">version</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">createdAt</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">snapshot</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">context</code> (if present),{" "}
      ...
      <li>
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">certificateHash</code> is NOT part of the
        signed payload.
      </li>
      <li>
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">meta</code> is NOT signed by the envelope.
      </li>
      <li>
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">receipt</code> is NOT signed by the envelope.
      </li>
    </ul>
    <p className="text-sm text-muted-foreground">
      Envelope failure indicates the signed bundle projection has been altered. It is reported separately from Layer 1
      and MUST NOT be conflated with integrity failure.
    </p>
    ...
    <div className="not-prose my-6 rounded-lg border border-amber-500/40 bg-amber-500/5 p-4">
      ...
      <div className="text-sm text-muted-foreground">
        When using public endpoints,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">snapshot.input</code> and{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">snapshot.output</code> are redacted. Therefore:
    ...
    <p>
      Envelope verification (Layer 3) is independent of integrity (Layer 1) and receipt (Layer 2). A bundle can be fully
      intact and properly attested while the envelope check returns
      <strong> FAIL</strong> or <strong>SKIPPED</strong>. The two outcomes carry different meaning and MUST NOT be
      conflated with integrity failure.
    </p>
    ...
    <ul>
      <li>
        ...
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono ml-1">
          meta.verificationEnvelope
        </code> and/or{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">meta.verificationEnvelopeSignature</code>, OR
        the verifier cannot reconstruct the signed projection (for example, fields required by the projection are not
        present in the received bundle). SKIPPED is NOT a failure.
      </li>
      <li>
        <strong>FAIL</strong> — the envelope is present and reconstructable, but the Ed25519 signature does not validate
        against the projected payload using the node key matched by
    ...
    <ol>
      <li>
        <strong>Mutation of a signed-but-unhashed field.</strong> The envelope signs an attestation projection (
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">attestationId</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">attestedAt</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">kid</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">nodeRuntimeHash</code>,{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">protocolVersion</code>) that is NOT covered by{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">certificateHash</code>. Editing one of those
        fields breaks Layer 3 while Layer 1 still passes.
      </li>
      <li>
      ...
      <li>
        <strong>
          Key mismatch (<code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">kid</code>).
        </strong>{" "}
        The verifier resolves the public key by{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">kid</code> from{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">
          node.nexart.io/.well-known/nexart-node.json
        </code>
        . If the published key set has rotated or the{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">kid</code> is unknown, the signature cannot be
        validated.
      </li>
      <li>
        <strong>Partial / redacted bundle.</strong> Public-safe representations may strip fields from{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">snapshot</code> or omit attestation projection
        fields. The envelope payload cannot be reconstructed bit-for-bit, so Layer 3 returns <strong>SKIPPED</strong>{" "}
        (or <strong>FAIL</strong> if the verifier attempts strict reconstruction). See "Why public bundles cannot always
        verify the envelope" below.
      </li>
      <li>
      ...
      <li>
        <strong>Pre-envelope artifact.</strong> Bundles produced before v0.16.1 may not include any envelope at all.
        Layer 3 returns <strong>SKIPPED</strong> by compatibility fallback. This is NOT a failure of integrity or
        attestation.
      </li>
    </ol>
    ...
    <p>
      Because <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> and the envelope
      cover different projections (the envelope additionally signs the attestation projection and excludes{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> itself), a Layer 3{" "}
      <strong>FAIL</strong> with Layer 1 <strong>PASS</strong> indicates the bundle's hashed content is intact, but the
      node's signed surface around it has been altered. Verifiers MUST report each layer independently. The aggregate
      verification status follows the rules in{" "}
    ...
    <p>
      ...
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">snapshot.output</code> are never exposed
      publicly; only their SHA-256 digests are. Some metadata fields may also be withheld depending on export settings.
      Because Layer 3 signs a strict whitelist projection that includes{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">snapshot</code> (and, when present,{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">context</code> /{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">contextSummary</code>), a verifier without the
      exact source bytes cannot reconstruct the signed payload byte-for-byte.
    </p>
    <p>
      In that case the public verifier returns <strong>SKIPPED</strong> for Layer 3 and still reports{" "}
      <strong>PASS</strong> for Layer 1 (against the public-safe representation) and Layer 2 (receipt validation against{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>). Full envelope
      verification requires the original, non-redacted CER bundle — typically held by the producer or an authorized
      auditor.
    </p>
    ...
    <p>
      If all applicable checks pass, the record is intact and its attestation is trustworthy. For a detailed breakdown,
      see{" "}
    ...
    <div className="not-prose my-6 rounded-lg border border-border bg-muted/30 p-4">
      ...
      <div className="text-sm text-muted-foreground">
        ...
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">POST /v1/admin/recertify-batch</code> to
        re-seal affected executions.
      </div>
    </div>

    <h2 id="how-to-verify">How to Verify a Record</h2>
    <p>
      There are three ways to verify a CER through the public verifier at{" "}
      <a href="https://verify.nexart.io" target="_blank" rel="noopener noreferrer">
        verify.nexart.io
      </a>
      .{" "}
      <strong>
        Prefer <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> for any
        persisted reference
      </strong>{" "}
      — it is the canonical identity of the artifact.{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">executionId</code> is convenience metadata
      only.
    </p>

    <h3 id="by-certificate-hash">1. By Certificate Hash (canonical)</h3>
    <p>
      If you have the <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>, use the
      hash-based URL. The colon in the hash must be URL-encoded:
    </p>
    <CodeBlock
    ...
    <p>
      You can also verify a record by uploading or pasting the full CER JSON bundle at{" "}
      <a href="https://verify.nexart.io" target="_blank" rel="noopener noreferrer">
        verify.nexart.io
      </a>
      . The verifier will recompute the hash and check the signature locally.
    </p>
    ...
    <p>
      The public verifier uses a <strong>redacted, public-safe representation</strong> of the record:
    </p>
    <ul>
      <li>Verification status, certificateHash, timestamp, and node identity are visible.</li>
      <li>
        Raw input and output content is <strong>never exposed</strong>. The record contains SHA-256 hashes of these
        fields, not the original text.
      </li>
    ...
    <ul>
      <li>
        The attestation node returns a <strong>signed receipt</strong> containing the certificateHash, timestamp, node
        identity, and Ed25519 signature. This is stored at{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation</code> in the CER bundle.
      </li>
      <li>
        A <strong>redacted, public-safe version</strong> of the record is persisted for verification.
      </li>
      <li>
        Input and output content is hashed (SHA-256). The hashes appear in the record, but the original content is not
        stored by the node or verifier.
      </li>
    ...
    <p>
      Each check returns <strong>PASS</strong>, <strong>FAIL</strong>, or <strong>SKIPPED</strong>:
    </p>
    <ol>
      <li>
        <strong>Bundle Integrity.</strong> Recompute the{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> from the bundle
        contents. If the hash differs, the bundle has been modified.
      </li>
      <li>
        <strong>Node Signature.</strong> Validate the Ed25519 signature using the public key published at{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">
          node.nexart.io/.well-known/nexart-node.json
        </code>
        , matched by the <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">kid</code> in the receipt.{" "}
        <strong>SKIPPED</strong> if no attestation is present.
      </li>
      <li>
        <strong>Receipt Consistency.</strong> Confirm the receipt at{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation.receipt</code> references
        the same <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> as the bundle
        and that the node identity matches. <strong>SKIPPED</strong> if no attestation is present.
      </li>
      <li>
        <strong>Verification Envelope.</strong> When present, validate{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.verificationEnvelopeSignature</code>{" "}
        against <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.verificationEnvelope</code>.
        This confirms the authoritative displayed verification surface has not been altered. <strong>SKIPPED</strong> if
        the bundle does not include a verification envelope.
      </li>
    </ol>
    <p className="text-sm text-muted-foreground">
      For a detailed breakdown of what each layer protects, see{" "}
      <Link to="/docs/ai-cer-verification-layers" className="text-primary hover:underline">
        AI CER Verification Layers
      </Link>
      .
    ...
    <p>
      The verification status reflects the overall outcome, as defined by the{" "}
      <Link to="/docs/cer-protocol" className="text-primary hover:underline">
        CER Protocol
      </Link>
      :
    </p>
...
NOT_FOUND     The requested execution record was not located.`}
    ...
    <ul>
      ...
      <li>
        ...
        outside the hash scope) is present. This is NOT a failure; supplemental fields are simply not cryptographically
        bound.
      </li>
      <li>
    ...
    <p>
      For the precise semantics of each result class, reseal handling, and the canonical role of{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> vs{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">executionId</code>, see{" "}
    ...
    <ul>
      <li>
        <strong>cer.ai.execution.v1</strong> (with attestation): all checks PASS → <strong>VERIFIED</strong>
      </li>
      <li>
        <strong>cer.ai.execution.v1</strong> (without attestation): bundleIntegrity PASS, attestation checks SKIPPED →{" "}
        <strong>VERIFIED</strong>
      </li>
      <li>
        <strong>signed-redacted-reseal</strong>: redacted reseal returned by the public verifier; validated against the
        NEW resealed <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> →{" "}
        <strong>VERIFIED</strong>
      </li>
      <li>
        <strong>hash-only-timestamp</strong>: only certificateHash is attested → <strong>VERIFIED</strong>
      </li>
      <li>
        <strong>legacy</strong>: older format, limited coverage → <strong>VERIFIED</strong> or <strong>FAILED</strong>{" "}
        depending on data
      </li>
    </ul>
    ...
    <ul>
      <li>
        <strong>projectHash integrity</strong>: recomputed from canonical content using{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">sha256-canonical-json</code>
      </li>
      <li>
        <strong>Per-step CER verification</strong>: each embedded CER's{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> is checked
        independently
      </li>
      <li>
        <strong>Project-level node receipt</strong>: validated if present
      </li>
    </ul>
    <p>
      Use <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">verifyProjectBundle()</code> (Node) or{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">verifyProjectBundleAsync()</code> (browser)
      from the SDK, or verify at{" "}
      <a href="https://verify.nexart.io" target="_blank" rel="noopener noreferrer">
        verify.nexart.io
      </a>{" "}
      via <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">/p/:projectHash</code>.
    </p>

    <h2 id="independent">Independent Verification (No API Required)</h2>
    <p>
      You can verify a CER without calling any NexArt API. No trust in NexArt infrastructure is required. All you need
      is the CER bundle (including{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation</code>) and access to the
      node's published keys:
    </p>
    <ol>
      <li>
      ...
      <li>
        ...
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation.receipt</code>
      </li>
      <li>
        Fetch the node's public key from{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">
          node.nexart.io/.well-known/nexart-node.json
        </code>
      </li>
      <li>
        Find the key matching the receipt's{" "}
    ...
    <p>
      If all steps pass, you can trust the attestation independently of NexArt infrastructure. No account, API key, or
      network call to NexArt is required beyond fetching the node's public key. For a from-scratch implementation guide
      in any language, see{" "}
...
protocolVersion = "1.3.0"   profile = "jcs-v1"      opt-in   (RFC 8785, standards-based)`}
    ...
    <p>
      <strong>1.3.0 → jcs-v1</strong> is opt-in. It uses JSON Canonicalization Scheme (RFC 8785) for both hashing and
      signing, providing a published, standards-based deterministic byte representation. Producers select it via{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">
    ...
    <p>
      ...
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion</code> is part of the
      attestation projection signed by the verification envelope, so it cannot be silently retargeted to a different
      profile without breaking Layer 3.
    </p>

    <h2 id="fail-closed">Fail-closed Behavior</h2>
    <p>
      Verification MUST fail closed. Any condition where a verifier cannot determine the correct rule to apply is
      reported as <strong>FAILED</strong>, never silently passed. SKIPPED is reserved for layers that are not applicable
      to the bundle as received (for example, an unsigned sealed bundle has no Layer 2 to run).
    </p>
    <ul>
      <li>
        Unknown <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">protocolVersion</code> →{" "}
        <strong>FAILED</strong>.
      </li>
      <li>
        Unknown <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">bundleType</code> →{" "}
        <strong>FAILED</strong>.
      </li>
      <li>
        Receipt <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">kid</code> not present in the published
        node key set → <strong>FAILED</strong> (Layer 2).
      </li>
      <li>
        Envelope present but a required projection field is missing or cannot be canonicalized → <strong>FAILED</strong>{" "}
        (Layer 3).
      </li>
      <li>
        Envelope absent → <strong>SKIPPED</strong> (Layer 3). Not a failure; the bundle simply does not carry that
        proof.
      </li>
    </ul>
    ...
    <p>
      A PASS result asserts exactly what the protocol attests, and nothing more. The following properties are explicitly
      out of scope:
    </p>
    <ul>
      <li>
        <strong>Completeness.</strong> Verification does not prove that the bundle represents every step of a larger
        workflow. Whether all relevant executions were captured is an integration concern.
      </li>
      <li>
        <strong>Semantic correctness.</strong> NexArt proves the bundle was produced as recorded. It does not assert
        that the model output was correct, appropriate, or compliant.
      </li>
      <li>
        <strong>Trusted (third-party) timestamping.</strong>{" "}
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">attestedAt</code> is a node-issued timestamp,
        bound to the bundle's <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">certificateHash</code> by
        the receipt signature. It provides an ordering guarantee relative to other records signed by the same node; it
        is NOT independent proof of existence at that wall-clock time. An external TSA (e.g. RFC 3161 via DigiCert) MAY
        be integrated alongside the receipt without modifying existing records.
      </li>
      <li>
        <strong>Public log inclusion.</strong> The current attestation log is node-internal. There is no public Merkle
        tree and no external witness at this protocol version. Verifiers MUST NOT infer transparency-log inclusion from
        a PASS result.
      </li>
      <li>
        <strong>Deterministic replay.</strong> Recomputing the same model output from the same inputs is only meaningful
        in controlled environments (fixed model version, seed, temperature 0, deterministic decoding). NexArt attests
        the recorded I/O; it does not assert reproducibility of the upstream model.
      </li>
    </ul>
...
# the flag does not change verification behavior.`}
    />
    <p>
      For the full verification contract, see the{" "}
      <Link to="/docs/cer-protocol" className="text-primary hover:underline">
        CER Protocol specification
      </Link>
      . For SDK functions, see{" "}
      <Link to="/docs/sdk" className="text-primary hover:underline">
        AI Execution SDK
      </Link>{" "}
      (sync and async modes). For browser-specific usage, see{" "}
