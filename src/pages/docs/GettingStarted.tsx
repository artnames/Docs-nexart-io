...
const GettingStarted = () => {
  return (
    <>
      ...
      <p>
        Four steps. Each has a single responsibility. Following them in order produces a record anyone can verify
        without trusting your infrastructure.
      </p>
      <GoldenPath />

      <h2 id="minimal-example">Minimal working example</h2>
      <p>
        One execution, one CER, one certification, one verification. This is the canonical example reused across the
        docs.
      </p>
      <MinimalExample />
      ...
      <p>
        NexArt creates <strong>Certified Execution Records (CERs)</strong>, cryptographically bound records of AI or
        deterministic execution that can be independently verified.
      </p>
      <p>
        Every CER produces a deterministic{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. That hash is the
        canonical identity of the record. Optional node attestation produces a signed receipt so anyone can verify the
        record without trusting your infrastructure.
      </p>

      <h2 id="paths">Two Integration Paths</h2>
      <p>
        NexArt supports both single-execution certification and multi-step workflow certification. Pick the one that
        matches your use case. You do not need Project Bundles for every integration.
      </p>
      ...
      <div className="not-prose my-6 grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <ul className="text-sm text-foreground/90 list-disc pl-4 space-y-1">
            ...
            <li>
              Seal a CER locally (<code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">sealCer()</code>) —
              offline, no API key
            </li>
            <li>Verify locally (integrity PASS, receipt &amp; envelope SKIPPED)</li>
            <li>
              Optional: certify via node and share{" "}
              <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">/c/{`{certificateHash}`}</code>
            </li>
          </ul>
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <ul className="text-sm text-foreground/90 list-disc pl-4 space-y-1">
            ...
            <li>
              Verify publicly at{" "}
              <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">verify.nexart.io</code>
            </li>
          </ul>
        </div>
      </div>
      ...
      <p className="text-sm text-muted-foreground">
        Current version:{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-xs font-mono">@nexart/ai-execution@0.23.0</code>.
      </p>

      <h2 id="path-a">Path A: Seal a Single CER</h2>
      <p>
        The simplest integration. One execution produces one verifiable record. Sealing is fully offline — no API key,
        no network call.
      </p>
      <CodeBlock
        language="typescript"
...
// report.checks.receiptConsistency === "SKIPPED"  (no envelope yet)`}
      />
      <p>
        The <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> is the canonical
        identity of the record. To make the bundle independently verifiable end-to-end (Layers 2 and 3 PASS), attest it
        via <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certifyAndAttestDecision()</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">attest(bundle, options)</code>, or{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">POST /v1/cer/ai/certify</code>. The{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> does not change.
      </p>
      <p className="text-sm text-muted-foreground">
        Note: <code className="bg-muted px-1.5 py-0.5 rounded text-xs font-mono">executionId</code> is not a unique
        artifact identifier. Always look up records by{" "}
      ...
      <p>
        For multi-step or multi-agent workflows, certify each step as its own CER and group them into a Project Bundle.
        The bundle has its own <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">projectHash</code>{" "}
        covering all step <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>{" "}
        values.
      </p>
      <CodeBlock
        language="typescript"
...
// bundle.integrity.projectHash is the verifiable hash`}
      />
      <p>
        For public verification on{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">verify.nexart.io</code>, the bundle must be
        registered on the node. See{" "}
      ...
      <p>
        ...
        and paste the <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>, or open
        the URL directly:
      </p>
      ...
      <div className="not-prose my-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <a
          href="https://github.com/artnames/nexart-langchain"
          target="_blank"
        ...
        <a
          href="https://github.com/artnames/nexart-n8n"
          target="_blank"
          rel="noopener noreferrer"
          className="block rounded-lg border border-border bg-card p-4 hover:border-primary/40 hover:bg-accent/30 transition-colors no-underline"
        >
