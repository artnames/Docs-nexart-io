...
const ContextSignals = () => (
  <div className="prose prose-invert max-w-none">
    ...
    <p>
      Context Signals are structured metadata recorded alongside an execution to capture external context. They let
      builders attach additional evidence such as policy check results, environment state, or authorization decisions to
      a Certified Execution Record.
    </p>

    <h2>Why They Exist</h2>
    <ul>
      <li>
        <strong>Logs are not verifiable.</strong> Traditional logging systems record events but provide no cryptographic
        proof that the logged data has not been modified.
      </li>
      <li>
        <strong>Governance systems lack structured evidence.</strong> Compliance workflows often depend on assertions
        without machine-readable evidence of what actually happened at execution time.
      </li>
      <li>
        <strong>Signals attach structured evidence to the record.</strong> They become part of the verifiable artifact
        (or are recorded alongside it) so reviewers can inspect what context was available when the execution ran.
      </li>
    </ul>
    ...
    <ul>
      <li>
        <strong>Optional.</strong> Signals are not required for CER creation. A record without signals is fully valid.
      </li>
      <li>
        <strong>Protocol-agnostic.</strong> Any structured metadata can be recorded. The protocol does not prescribe
        specific signal types.
      </li>
      <li>
        <strong>Scope varies.</strong> A signal MAY be included in the{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> (hash-bound) or
        recorded as supplemental context (outside the hash scope). Which mode applies depends on the artifact type and
        how the CER was produced.
      </li>
      <li>
      ...
      <li>
        <strong>Supplemental signals do not block core verification.</strong> Their absence or modification does not
        cause core artifact verification to fail. The verifier may surface them as supplemental evidence.
      </li>
    </ul>
    ...
    <div className="not-prose my-6 rounded-lg border border-border bg-muted/30 p-4">
      ...
      <div className="text-sm text-muted-foreground">
        Both modes are valid. A signal being outside the hash scope does NOT mean core artifact integrity has failed. It
        means that signal is recorded for context, not as part of the cryptographic commitment. See{" "}
...
console.log(result.certificateHash);`}
    ...
    <CodeBlock
      ...
      code={`npx @nexart/cli@1.1.0 ai create execution.json --signals-file signals.json`}
    />
    <p>
      ...
      code={`[
  {
    ...
    <h2>Verification</h2>
    <p>How signals affect verification depends on whether they are hash-bound or supplemental:</p>
    <ul>
      <li>
        <strong>Hash-bound signals</strong> are part of the data covered by the{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. Any modification
        causes <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">bundleIntegrity</code> to FAIL.
      </li>
      <li>
        <strong>Supplemental signals</strong> are recorded alongside the artifact but outside the hash. Their absence or
        modification does NOT cause core verification to fail. They may be surfaced by the verifier as supplemental
        evidence.
      </li>
    </ul>
    <p>
      ...
      for how signal scope is reported in verification results.
    </p>

    <div className="not-prose my-6 rounded-lg border border-border bg-muted/30 p-4">
      <div className="text-sm font-medium text-foreground mb-1">Important</div>
      <div className="text-sm text-muted-foreground">
        NexArt records signals but does not interpret or enforce them. The protocol treats signals as opaque metadata.
        Any semantic meaning or enforcement logic is the responsibility of the consuming application.
      </div>
    </div>
    ...
    <ul>
      <li>
        ...
        (collector, ExecutionContext, snapshots, builder)
      </li>
      <li>
        <Link to="/docs/concepts/execution-context" className="text-primary hover:underline">
          Execution Context and Signals
        </Link>
        : builder API using <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">@nexart/signals</code>{" "}
        (createContext, ctx.input, ctx.tool, ctx.decision, ctx.output, ctx.wrap)
      </li>
      <li>
        <Link to="/docs/sdk" className="text-primary hover:underline">
          AI Execution SDK
        </Link>
        : API reference for the <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">signals</code>{" "}
        parameter
      </li>
      <li>
        ...
        : use <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">--signals-file</code> from the command
        line
      </li>
      <li>
        <Link to="/docs/verification-semantics" className="text-primary hover:underline">
