...
const CER = () => (
  ...
  "snapshot": {
    ...
    <p>Identity fields are <strong>claims captured at execution time</strong>, not independently verifiable assertions. Only hashes are stored in the CER (<code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">emailHash</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">assertionHash</code>). NexArt does NOT verify identity providers and does NOT validate IdP assertions. Identity verification (token validation, signature checks, audience and expiry checks) happens in the application layer before the CER is sealed. NexArt seals the provided <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">identity</code> object as tamper-evident evidence of what the application asserted at execution time. The <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">verified</code> flag reflects the application's claim, not an independent NexArt check. Identity verification is evolving toward externally verifiable assertions in a future protocol version.</p>
