...
const FAQ = () => (
  <>
    ...
    <h2 id="default-1-2-0">Why does NexArt default to protocolVersion 1.2.0 if I omit protocolVersion?</h2>
    <p>Because older clients and existing records were created before explicit <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">1.3.0</code> / JCS support. Defaulting omitted or <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">null</code> <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion</code> to <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">1.2.0</code> preserves backward compatibility and keeps legacy records verifiable. New integrations that require RFC 8785 JCS canonicalization should set <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion: "1.3.0"</code> explicitly at creation/sealing time. See <Link to="/docs/verification-model#protocol-version" className="text-primary hover:underline">Verification Model → protocolVersion</Link>.</p>

    <h2 id="how-jcs">How do I use RFC 8785 JCS?</h2>
    <p>Create the CER with <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion: "1.3.0"</code>. Pass it into <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">createSnapshot</code> (or the equivalent SDK entry point such as <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certifyDecision</code>) so it is part of the sealed snapshot. <strong>Do not add or change it after sealing</strong> — it controls the canonicalization profile (<code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">jcs-v1</code>) used to compute the <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. The CLI equivalent is <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">--protocol-version 1.3.0</code>. See the <Link to="/docs/sdk#seal" className="text-primary hover:underline">SDK reference</Link> for a code example.</p>

    <h2 id="unsupported-version">What happens if I send protocolVersion "2.0.0" or another unsupported version?</h2>
    <p>The node rejects unsupported explicit protocol versions before signing. Verification also <strong>fails closed</strong> on unknown <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion</code> values. This prevents unsupported records from being accidentally attested or silently accepted by verifiers.</p>

    <h2 id="root-endpoint-version">Why does the root node endpoint still show protocolVersion "1.2.0"?</h2>
    <p>That value represents the node's default/backward-compatible protocol version for clients that do not specify one. It does <strong>not</strong> mean <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">1.3.0</code> is unsupported. For an individual CER, check the sealed snapshot, the node receipt, and the verification envelope attestation. A <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">1.3.0</code> CER will carry <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion: "1.3.0"</code> in those signed surfaces; the node mirrors it from the sealed snapshot rather than rewriting it on the UI side. NexArt proves the integrity and authenticity of the execution record, not the correctness of the AI result.</p>

    <h2 id="change-protocol-version">Can I change protocolVersion after creating the CER?</h2>
    <p>No. <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion</code> sits inside the sealed snapshot and is part of the strict whitelist projection that produces the <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. Changing it after sealing changes what is being verified and invalidates the record. Set it correctly at creation/sealing/certification time. See <Link to="/docs/concepts/hashes" className="text-primary hover:underline">Hashes</Link> and <Link to="/docs/attestation-node" className="text-primary hover:underline">Attestation Node</Link>.</p>
