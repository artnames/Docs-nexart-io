...
const Architecture = () => (
  <>
    ...
    <p>
      The fastest way to validate an integration against the canonical pipeline. One script, one set of environment
      variables, one expected output: three PASS lines for Integrity, Receipt, and Envelope.
    </p>
    <TestHarness />

    <h2 id="audience">Audience and scope</h2>
    <p>
      This page is the developer reference for the NexArt protocol. It defines the exact structure of a Certified
      Execution Record (CER), the inputs to the certificateHash, the verification layers, and the invariants that all
      producers, attestation nodes, and verifiers MUST honor. It is intentionally precise and aligned with the protocol.
      Where RFC keywords (MUST, MUST NOT, SHOULD) appear, they carry their normal protocol meaning.
    </p>

    <h2 id="ownership">Component ownership boundaries</h2>
    <p>
      The system has three components with strict, non-overlapping responsibilities. The CLI contains{" "}
      <strong>zero CER cryptographic logic</strong>. All hashing, canonicalization, and verification is implemented in
      the SDK.
    </p>
    <ul>
      <li>
      ...
      <li>
        <strong>CLI</strong> (
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">@nexart/cli@0.16.1</code>) owns: the command
        ...
        and output formatting. It delegates every cryptographic operation to the SDK.
      </li>
      <li>
        <strong>Node</strong> (attestation node) owns: bundle attestation (
        <code className="bg-muted px-1 py-0.5 rounded text-xs font-mono">POST /v1/cer/ai/certify</code>), Ed25519
        receipt signing, verification envelope signature, and public key publication at{" "}
    ...
    <p>
      The pipeline has five stages. Stages 1 to 3 are local to the producer (SDK or CLI). Stage 4 is optional and adds
      node attestation. Stage 5 is independent and may be performed by anyone. The canonical workflow is:{" "}
      <strong>create input → seal → verify → (optional) certify → verify</strong>.
    </p>

    <h3 id="stage-1">1. Execution capture</h3>
    <p>
      The producer executes an AI call or a deterministic computation and captures the observable evidence: model
      identity, inputs, outputs, parameters, and any contextual signals. Inputs and outputs are not stored in the bundle
      directly; they are reduced to SHA-256 digests (
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">inputHash</code>,{" "}
    ...
    <p>
      Capture is the producer's responsibility. NexArt does not guarantee completeness of what was captured. It
      guarantees that what was captured cannot be altered without detection.
    </p>

    <h3 id="stage-2">2. CER creation (local sealing)</h3>
    <p>
      The producer assembles the canonical CER bundle and computes the{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>. This stage is fully
      offline: it requires no network access and no API key. In SDK form this is{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">sealCer()</code>; in CLI form it is{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">nexart ai seal</code>. The output is a{" "}
      <strong>sealed</strong> CER bundle (integrity only; no attestation). Required top-level fields:
    </p>
    <ul>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">bundleType</code> - constant string{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">"cer.ai.execution.v1"</code> for AI
        executions.
      </li>
      <li>
      ...
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">snapshot</code> - object containing{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">model</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">inputHash</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">outputHash</code>, and{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">metadata</code> (e.g.{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">appId</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">projectId</code>).
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">context</code> - optional. Structured signals
        included in the hash when present. See{" "}
      ...
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">contextSummary</code> - optional, summary of
        context. Included in the hash when present.
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">policyEvaluation</code> - optional. Captured
        policy decision result. Included in the hash when present.
      </li>
    </ul>
    ...
    <p className="text-sm text-muted-foreground">
      A sealed bundle (Stage 2) is already a valid CER. Certification is optional and adds independently verifiable node
      attestation. After certification, the bundle is
      <strong> certified</strong> rather than only sealed.
    </p>
    <p>
      The producer may submit the bundle to an attestation node (
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">POST /v1/cer/ai/certify</code>). The node
      validates the bundle and issues a deterministic, Ed25519-signed receipt that references the bundle's
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">certificateHash</code>. The receipt and
      signature are stored at
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">bundle.meta.attestation</code> with fields{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">receipt</code>,{" "}
    ...
    <p>
      ...
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">certificateHash</code> MUST remain
      unchanged after attestation.
    </p>

    <h3 id="stage-5">5. Verification</h3>
    <p>
      Any party with the bundle and the node's published public keys can verify. The verifier runs up to four checks;
      each returns <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">PASS</code>,{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">FAIL</code>, or{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">SKIPPED</code>. The overall verification status
      is <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">VERIFIED</code>,{" "}
  ...
  "meta": {
    ...
    <ul>
      <li>
        <strong>bundleType</strong> - REQUIRED. Constant. Identifies the bundle schema. For AI executions, exactly{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">"cer.ai.execution.v1"</code>. Hashed.
      </li>
      <li>
        <strong>version</strong> - REQUIRED. Bundle schema version. Hashed. Verifiers MUST refuse to upgrade or
        downgrade the value.
      </li>
      <li>
        <strong>createdAt</strong> - REQUIRED. ISO 8601 UTC timestamp at the moment of CER creation. Hashed.
      </li>
      <li>
        <strong>snapshot</strong> - REQUIRED. Hashed. Carries{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">model</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">inputHash</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">outputHash</code>, and{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">metadata</code>. Sub-fields are stable;
        producers MUST NOT inject mutable runtime data into snapshot.
      </li>
      <li>
        <strong>context</strong> - OPTIONAL. Hashed when present. Used for structured signals that bind to the
        certificateHash.
      </li>
      <li>
        <strong>contextSummary</strong> - OPTIONAL. Hashed when present. Human-readable companion to{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">context</code>.
      </li>
      <li>
        <strong>certificateHash</strong> - REQUIRED in the persisted bundle. Excluded from its own hash input.
      </li>
      <li>
        <strong>meta</strong> - OPTIONAL. Excluded from hash. Carries attestation, verification envelope, declaration,
        and any non-normative metadata.
      </li>
    </ul>
    ...
    <ul>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">bundleType</code>
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">version</code>
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">createdAt</code>
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">snapshot</code>
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">context</code> (only if present in bundle)
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">contextSummary</code> (only if present in
        bundle)
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">policyEvaluation</code> (only if present in
        bundle)
      </li>
    </ul>
    ...
    <ul>
      ...
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta</code> and all subfields, including{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.verificationEnvelope</code>,{" "}
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.verificationEnvelopeSignature</code>
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">declaration</code>
      </li>
      <li>
        <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">receipt</code> (top-level convenience copy,
        when present)
      </li>
    ...
    <p>
      The verifier projects the received bundle to the whitelist, canonicalizes with JCS, and computes SHA-256. The
      result MUST equal the bundle's
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">certificateHash</code>. Any difference is{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">FAIL</code>. This layer is always evaluated.
    </p>

    <h3 id="layer-2">Layer 2 - Receipt signature (Node Signature + Receipt Consistency)</h3>
    <p>
      When <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation</code> is present, the
      verifier:
    </p>
    <ol>
      <li>
        Resolves the node public key by <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">kid</code>{" "}
        from{" "}
      ...
      <li>
        Confirms <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">receipt.certificateHash</code>{" "}
        equals the bundle's <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>.
      </li>
    </ol>
    <p>
      If <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta.attestation</code> is absent, both
      checks return
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">SKIPPED</code>. Skipping is not a failure
      of integrity.
    </p>
    ...
    <p>
      ...
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">Deleted</code>) are recorded in registry state
      external to the bundle. Republishing a modified bundle produces a new
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">certificateHash</code> and is a different
      record.
    </p>

    <h3 id="idempotency">Idempotency</h3>
    <p>
      Given identical inputs to the canonicalization and hashing pipeline, every conformant implementation MUST produce
      the same
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono ml-1">certificateHash</code>. Re-issuing the
      same execution capture MUST yield bit-identical hashes. Producers SHOULD treat{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code> as the deduplication key
    ...
    <p>
      Verification MUST be possible without contacting NexArt's APIs. The bundle plus the node's published public keys
      are sufficient. The
      <Link to="/docs/verify-nexart" className="text-primary hover:underline ml-1">
        verify.nexart.io
      </Link>{" "}
    ...
    <p>
      Two valid CER states. <strong>Sealed</strong> is produced offline by the SDK or CLI and proves integrity only.{" "}
      <strong>Certified</strong> adds an Ed25519 receipt and a verification envelope from the attestation node, making
      the bundle independently verifiable end-to-end. Certification does not change the{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>.
    </p>
    <SealedVsCertified />
    ...
    <ul>
      <li>
        <strong>Execution integrity</strong> — the recorded execution cannot be modified after sealing without breaking
      ...
      <li>
        ...
        published public keys. No NexArt API access required.
      </li>
    </ul>
    ...
    <ul>
      <li>
        <strong>Output correctness</strong> — NexArt does not validate whether the captured output is right, useful, or
        safe.
      </li>
      <li>
        <strong>Model validity</strong> — NexArt does not certify the model, its weights, or its behavior.
      </li>
      <li>
        <strong>Deterministic replay</strong> — NexArt does not guarantee that re-running the same input produces the
        same output. It records what happened; it does not reproduce it.
      </li>
    </ul>
    ...
    <ul>
      ...
      <li>
        <Link to="/docs/concepts/hashes" className="text-primary hover:underline">
          certificateHash vs projectHash
        </Link>{" "}
        - hash scope and algorithm.
      </li>
      <li>
      ...
      <li>
        <Link to="/docs/attestation-node" className="text-primary hover:underline">
          Attestation Node
        </Link>{" "}
        - node contract and key publication.
      </li>
      <li>
        <Link to="/docs/verification-semantics" className="text-primary hover:underline">
          Verification Semantics
        </Link>{" "}
        - status mapping and error codes.
      </li>
    </ul>
  </>
);
