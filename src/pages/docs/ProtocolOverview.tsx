...
const ProtocolOverview = () => (
  <>
    ...
    <p>A SHA-256 hash of a strict whitelist projection of the CER bundle, canonicalized per the profile bound to <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion</code> (1.2.0 → nexart-v1, default; 1.3.0 → jcs-v1 / RFC 8785, opt-in; 1.3.1 → jcs-v1 with node-side commitment envelopes for <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">input</code> and <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">output</code>). It is computed ONLY over <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">bundleType</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">version</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">createdAt</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">snapshot</code>, and (if present) <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">context</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">contextSummary</code>, and <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">policyEvaluation</code>. <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">certificateHash</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">meta</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">declaration</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">verificationEnvelope</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">verificationEnvelopeSignature</code>, <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">receipt</code>, and any unknown fields are NOT hashed. Verifiers MUST apply the whitelist projection to the bundle exactly as received and use the canonicalization profile corresponding to the bundle's <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">protocolVersion</code>.</p>

    <h3>Confidential Execution (Protocol 1.3.1)</h3>
    <p>
      Protocol <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">1.3.1</code> introduces
      commitment-based confidentiality, replacing the hash-bound omission model used in
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono"> 1.2.0</code>. Clients send raw
      {" "}<code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">prompt</code>,{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">input</code>,{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">output</code>, and{" "}
      <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">parameters</code> to the node.
      The node seals <code className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono">input</code> and{" "}
